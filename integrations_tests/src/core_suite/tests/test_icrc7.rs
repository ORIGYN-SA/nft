...
fn test_icrc7_atomic_batch_transfers() {
    ...
    let atomic_batch_transfers = icrc7_atomic_batch_transfers(
        pic,
        controller,
        collection_canister_id,
        &()
...
fn test_icrc7_transfer() {
    ...
    match mint_return {
        Ok(token_id) => {
            ...
            assert_eq!(owner_of[0], Some(Account { owner: nft_owner2, subaccount: None }));
        }
        Err(e) => {
            println!("Error minting NFT: {:?}", e);
            assert!(false);
...
fn test_icrc7_transfer_no_argument() {
    ...
    assert_eq!(
        transfer_response[0].as_ref().unwrap().as_ref().err().unwrap().1,
...
fn test_icrc7_transfer_exceed_max_batch_size() {
    ...
    let max_update_batch_size = icrc7_max_update_batch_size(
        pic,
        controller,
        collection_canister_id,
        &()
    ...
    let transfer_args: Vec<icrc7::TransferArg> = (0..u64
        ::try_from(max_update_batch_size.0)
        .unwrap() + 1)
    ...
    match mint_return {
        Ok(token_id) => {
            let transfer_args = icrc7::TransferArg {
                ...
                token_id: token_id.clone(),
                memo: Some(ByteBuf::from(vec![0; icrc7::DEFAULT_MAX_MEMO_SIZE as usize + 1])),
                from_subaccount: None,
                created_at_time: None,
            };
...
fn test_icrc7_max_query_batch_size() {
    ...
    let max_query_batch_size = icrc7_max_query_batch_size(
        pic,
        controller,
        collection_canister_id,
        &()
...
fn test_icrc7_tokens_of() {
    ...
    let tokens_of_owner1 = icrc7_tokens_of(
        ...
        controller,
        collection_canister_id,
        &(Account { owner: nft_owner1, subaccount: None }, None, None)
    );
    ...
    let tokens_of_owner2 = icrc7_tokens_of(
        ...
        controller,
        collection_canister_id,
        &(Account { owner: nft_owner2, subaccount: None }, None, None)
    );
...
fn test_icrc7_balance_of() {
    ...
    let balance_of_owner1 = icrc7_balance_of(
        ...
        controller,
        collection_canister_id,
        &vec![Account { owner: nft_owner1, subaccount: None }]
    );
    ...
    let balance_of_owner2 = icrc7_balance_of(
        ...
        controller,
        collection_canister_id,
        &vec![Account { owner: nft_owner2, subaccount: None }]
    );
