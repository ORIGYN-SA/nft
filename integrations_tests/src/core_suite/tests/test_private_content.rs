...
use core_nft_common::construct_canonical_identity;
...
fn test_private_content_encryption_and_decryption_with_vetkeys() {
    ...
    let context_bytes = nft_owner1.as_slice(); // "canonicalized principal"
                                               // TODO: there can be two methods which derive key:
                                               // - Passing context as input
                                               // - Automise context generation on the canister side"
                                               //      1. check accesses
                                               //      2. generate canonicalized principal
    ...
    // -------------------------------------------------------------------------
    let pub_key_args = derive_vetkey_public_key::Args {
        context: ByteBuf::from(context_bytes),
    };

    let pub_key_response =
        derive_vetkey_public_key(pic, controller, collection_canister_id, &pub_key_args)
    ...
    let derive_args = derive_vetkey::Args {
        context: ByteBuf::from(context_bytes),
        input: ByteBuf::from(context_bytes), // Requesting key specifically for this salted context
    ...
    let vetkey = encrypted_vetkey
        .decrypt_and_verify(&tsk, &dpk, &context_bytes)
    ...
    // -------------------------------------------------------------------------
    // Keeping hash, plaintext_size, and total_size tracked to plaintext as requested.
    ...
    let init_args = core_nft_api::init_private_content_upload::Args {
        ...
        plaintext_size,
        expected_chunks: 1,
        chunk_size: Some(ciphertext.len() as u64), // Chunk size handles payload container dimensions
    ...
    let private_entry = PrivateEntry {
        ...
        plaintext_size,
        encryption_mode: EncryptionMode::AES256,
        canonical_identity: context_bytes.to_vec().clone(),
        previous_canonical_identity: None,
        storage_canister_id: collection_canister_id,
        storage_path: storage_path.clone(),
        pending_upload: None,
