...
impl FromStr for WrappedAccount {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // Try to parse as a Principal first
        if let Ok(principal) = Principal::from_str(s) {
            return Ok(WrappedAccount(Account {
                owner: principal,
                subaccount: None,
            }));
        }

        // If it's not a valid Principal, return an error
        Err(format!("Invalid account string: {}", s))
    }
}
