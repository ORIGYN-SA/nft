...
pub trait SubCanister {
    ...
    async fn create_canister(&mut self) -> Result<Box<impl Canister>, NewCanisterError>;
...
pub struct SubCanisterManager<U, T>
    where T: Canister + Clone + Serialize + Debug, U: Clone + Serialize + CandidType {
    pub init_args: U,
    pub upgrade_args: U,
    ...
    where T: Canister + Clone + Serialize + Debug, U: Clone + Serialize + CandidType
{
    ...
    pub async fn create_canister(
        &mut self
    ) -> Result<Box<impl Canister + Debug + Clone>, NewCanisterError> {
        ...
        trace(&format!("Installing code to canister with principal : {}", canister_id));
        trace(&format!("Install args : {:?}", install_args));
    ...
    pub async fn update_canisters(&mut self) -> Result<(), Vec<String>> {
